...
<script>
		...
		layui.use(['table','laypage'], function() {
			...
			table.render({
    			...
    			,cols: [[
					{type:'checkbox', fixed: 'left'}
	     			,{field:'fb_id' ,width:80, title: '序号', sort: true}
			...
			table.on('tool(test1)', function(obj) {
				...
				if(obj.event === 'detail') {
					console.log(obj);
					layer.msg('ID：' + data.feenback_id + ' 的查看操作');
				} else if(obj.event === 'del') {
					layer.confirm('真的删除行么', function(index) {
						$.ajax({url:"Feenbackitem_delete.action?feenback.feenback_id="+obj.data.feenback_id,async:true,success:function(result){
						}});
						obj.del();
						layer.close(index);
					});
					...
					getCheckData: function() { //删除选中数据
						...
						layer.confirm('真的删除行么', function(index) {
							for(var i=0;i<data.length;i++){
								$.ajax({url:"Feenbackitem_delete.action?feenback.feenback_id="+data[i].feenback_id,async:true,success:function(result){
								}});
							}
